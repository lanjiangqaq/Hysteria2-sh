...
## 端口跳跃 (Port Hopping) 防 QoS 说明

为了有效防御 ISP（网络运营商）针对单一端口的深度包检测（DPI）与 QoS 限速，本脚本集成了 **iptables 端口转发机制**。

* **功能原理**：通过在服务器端配置 NAT 转发规则，将客户端发往指定范围内（例如 `30000-50000`）的 UDP 流量，实时重定向至 Hysteria 2 的真实监听端口。
* **主要优势**：
    * **抗干扰**：通过随机分布在范围内的端口进行通信，大幅降低固定端口被运营商标记或限速的概率。
    * **自动清理**：在脚本安装时自动配置并持久化转发规则，在执行脚本的“彻底卸载”选项时，所有防火墙规则会自动被精准清理，不留任何系统垃圾。
* **使用建议**：在执行安装脚本时，当系统询问“是否启用端口跳跃功能”时，输入 `y` 并设置一个较大的端口范围（建议范围跨度至少在 5000 以上），以获得最佳的防封锁效果。
