...
get_user_input() {
    ...
    echo -e "${CYAN}===== 带宽设置 (用于 QUIC 拥塞窗口优化) =====${RESET}"
    echo "说明: 准确填写带宽可让 Hysteria2 更快达到峰值速率，尤其是高延迟链路。留空则使用自适应 BBR 探测。"
    read -rp "请输入上行带宽 (单位 mbps，留空则自适应): " BANDWIDTH_UP
    read -rp "请输入下行带宽 (单位 mbps，留空则自适应): " BANDWIDTH_DOWN
    echo -e "${CYAN}==============================================${RESET}"
}
...
tune_kernel_network() {
    echo -e "${YELLOW}正在检测并优化内核网络参数以提升单连接吞吐...${RESET}"
    ...
    SYSCTL_FILE="/etc/sysctl.d/99-hysteria2-tuning.conf"
    cat > "$SYSCTL_FILE" << 'EOF'
# Hysteria2 / QUIC 高延迟链路优化
...
install_hysteria2() {
    ...
    # QUIC 接收窗口: 固定采用较大的推荐值，而非随机取值，
    # 在高延迟链路 (高 BDP) 下能更快打满带宽，且避免随机值偏小拖累吞吐。
    STREAM_RW=33554432   # 32 MiB 单流窗口
    CONN_RW=67108864     # 64 MiB 连接总窗口
...
EOF
        if [ -n "$BANDWIDTH_UP" ] || [ -n "$BANDWIDTH_DOWN" ]; then
            echo ""
            echo "bandwidth:"
            [ -n "$BANDWIDTH_UP" ] && echo "  up: ${BANDWIDTH_UP} mbps"
            [ -n "$BANDWIDTH_DOWN" ] && echo "  down: ${BANDWIDTH_DOWN} mbps"
        fi
